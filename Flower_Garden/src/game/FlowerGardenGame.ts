...
export class FlowerGardenGame {
  ...
  ensureStateSchema() {
    this.gardenerIds.forEach((id) => {
      ...
      if (!this.state.gardeners[id].barnSeeds) this.state.gardeners[id].barnSeeds = {};
      Object.keys(this.seedTypes).forEach((seed) => {
        if (typeof this.state.gardeners[id].seeds[seed] !== "number") this.state.gardeners[id].seeds[seed] = 0;
        if (typeof this.state.gardeners[id].barnSeeds[seed] !== "number") this.state.gardeners[id].barnSeeds[seed] = 0;
      });
    });
    this.persist();
  }
  ...
  tryEnterOwnBarn(force = false) {
    if (this.player.insideBarn) return;
    const barnX = this.scene.getBarnX("you");
    if (barnX == null) return;
  ...
  barnDeliveryCycle() {
    this.gardenerIds.forEach((id) => {
      if (Math.random() < 0.55) this.addBarnSeedDelivery(id, 1 + Math.floor(Math.random() * 2));
    });
    this.persist();
    this.ui.renderInventory();
    if (this.player.insideBarn) this.ui.renderBarnStorage("you");
    const total = Object.values(this.state.gardeners.you.barnSeeds).reduce((sum, value) => sum + value, 0);
    if (total > 0) this.ui.setStatus(`New seed delivery reached your barn. Stored seeds: ${total}.`);
  }

  npcBarnCollectionCycle() {
    this.npcIds.forEach((ownerId) => {
      if (this.scene.isNpcInsideBarn(ownerId)) return;
      const gardener = this.state.gardeners[ownerId];
      const stored = Object.values(gardener.barnSeeds).reduce((sum, qty) => sum + qty, 0);
      if (stored <= 0 || Math.random() > 0.55) return;
  ...
  waveToNearestGardener() {
    if (this.player.waveCooldownMs > 0) return;
  ...
  setupInputs() {
    window.addEventListener("keydown", (event) => {
      ...
      if (key === "b" && !typing) {
        event.preventDefault();
        if (this.player.insideBarn) this.exitBarn();
        else this.tryEnterOwnBarn();
      }
    ...
    this.dom.garden.addEventListener("click", (event) => {
      ...
      if (barn) {
        ...
        if (owner === "you") this.tryEnterOwnBarn(true);
        else this.ui.setStatus("You can only enter your own barn.");
        return;
      }
      if (targetEl && targetEl.classList.contains("coin")) return;
      if (this.player.insideBarn) return;
    ...
    if (this.dom.closeInstructions && this.dom.instructionsModal) {
      ...
      this.dom.instructionsModal.addEventListener("click", (event) => {
        if (event.target === this.dom.instructionsModal) this.dom.instructionsModal.style.display = "none";
      });
    }
    ...
    if (this.dom.touchControls) {
      ...
      this.dom.touchControls.querySelectorAll("[data-touch-action]").forEach((btn) => {
        btn.addEventListener("click", (event) => {
          ...
          if (action === "plant" && !this.player.insideBarn) this.growth.plantAtPlayer();
          if (action === "water" && !this.player.insideBarn) this.growth.waterNearestPlant();
          if (action === "wave" && !this.player.insideBarn) this.waveToNearestGardener();
          if (action === "barn") {
            if (this.player.insideBarn) this.exitBarn();
            else this.tryEnterOwnBarn();
          }
          if (action === "help" && this.dom.instructionsModal) this.dom.instructionsModal.style.display = "flex";
    ...
    if (this.dom.closeBarn && this.dom.barnModal) {
      ...
      this.dom.barnModal.addEventListener("click", (event) => {
        if (event.target === this.dom.barnModal) this.exitBarn();
      });
    }
  ...
  updateMovement = (now) => {
    if (!this.updateMovement.last) this.updateMovement.last = now;
    ...
    if (!typing) {
      ...
      if (this.keys.w || this.keys.arrowup) md -= 1;
      if (this.keys.s || this.keys.arrowdown) md += 1;
    }
    ...
    if (this.player.plantedCooldownMs > 0) this.player.plantedCooldownMs -= dt * 1000;
    if (this.player.waveCooldownMs > 0) this.player.waveCooldownMs -= dt * 1000;
    if (this.player.zoneWarnMs > 0) this.player.zoneWarnMs -= dt * 1000;
