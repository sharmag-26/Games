...
export class SceneSystem {
    ...
    sendNpcInsideBarn(ownerId, durationMs = 1800, onReturn = null) {
        if (this.isNpcInsideBarn(ownerId))
            return false;
        ...
        if (this.npcMotion[ownerId]) {
            ...
            if (barnX != null)
                this.npcMotion[ownerId].x = barnX;
            this.npcMotion[ownerId].depth = 0.18;
        }
        setTimeout(() => {
            if ((this.npcInsideBarnUntil[ownerId] || 0) > Date.now())
                return;
    ...
    npcHasPendingTarget(ownerId) {
        ...
        if (!motion)
            return false;
        return (typeof motion.targetX === "number" ||
    ...
    updateNpcMotion(dt) {
        ...
        npcEls.forEach((el) => {
            const id = el.dataset.id;
            if (!id || !this.game.plotBounds[id] || !this.npcMotion[id])
                return;
            if ((this.npcInsideBarnUntil[id] || 0) > now)
                return;
            const bounds = this.game.plotBounds[id];
            const pad = 26;
